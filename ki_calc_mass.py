...
def calc_masseq(file_list, norms): #calculate masseq score for each file in file_list and write to csv file
	...
	for i in file_list:
		...
		incluion = event_inc.groupby('event').sum() #sum counts for each gene 
		...
		psi = incluion['counts']/(incluion['counts'] + exclusion['counts']) #calculate psi
		print(psi)
		mas = (psi-norm['med'])/(norm['per']-norm['med']) #calculate masseq score for each gene using psi and norm values
		#print(mas)
		score = sum(mas)/22
		#add first 5 values of masseq to get score, divide by 5
		#score = sum(mas[0:5])/5
		#print(score)
		masseq.loc[ind, 'Masseq'] = score #add score to data frame using index as column name
		print(masseq)
		
	masseq = masseq.sort_values(by = 'Masseq') #sort data frame by masseq score values
	masseq.to_csv('masseq_scores.csv') #write data frame to csv
...
def psi_calculation(file_list): #calculate psi values for each file in file_list and write to csv file
	...
	for i in file_list: #loop through each file in file_list and calculate psi values for each gene
		...
		event_inc = event_counts[event_counts.events.str.contains('POS')] #define inclusion event / isolate gene name 
		event_inc['event'] = event_inc.events.str.split("_").str[0]
		incluion = event_inc.groupby('event').sum()

		event_exc = event_counts[event_counts.events.str.contains('NEG')] #define exclusion event / isolate gene name
		event_exc['event'] = event_exc.events.str.split("_").str[0] 
		exclusion = event_exc.groupby('event').sum()
	
		psi[ind] = incluion['counts']/(incluion['counts'] + exclusion['counts']) #calculate psi using index as column name
		print(psi)
